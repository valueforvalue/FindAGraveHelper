...
- **Issue #137: B10-pre1851-tight strategy + unknown-state scoring bias.** Two new fallback paths for the v5 strategy ladder: (1) **`B10-pre1851-tight`** is a new function-form strategy in `scripts/search/strategies.py` that fires only when `pensioner.birth_year < 1851` (rough CW-era source years, ~15% of the 7,758 cohort). Uses `birthyearfilter=3` — tighter than B1's 1-window (irrelevant for pre-1851) and B3/B4's 5-window (over-accepts on rough years). Positioned after B5, before C1 in the ladder per issue acceptance criteria. (2) **Unknown-state scoring bias** in `scripts/fag/scoring.py` — when BOTH the pensioner has no known state AND the candidate's burial state is empty, adds +0.05 to the score and surfaces `state_bias: 0.05` in the breakdown. Sized small to break ties among same-name candidates without overpowering name/death evidence. Note: the issue originally framed both as function-form strategies, but FaG's URL surface has no param that filters "burial state = empty" — `locationId` only filters TO a state — so Strategy A had to land as a scoring-side bias per the codebase's `Strategy` Protocol (URL-only). Tests pin: B10 guard matrix (fires for 1850, skips for 1851+), ladder position, both-states-empty bias trigger, single-side-known no-bias, bias doesn't override name signal. Suite: **1,704 passed** (was 1,688 before #137 work; +16 new tests across `test_strategies.py`, `test_fag_engine.py`, and new `test_state_bias_137.py`). `test_ladder_has_13_strategies` bumped to 14; `test_strategies_module_has_10_public_strategies` bumped to 11.

- **Issue #137 follow-up: full 575-record probe + 4 miss-class diagnosis.** Shipped #137 with B10 + bias, then validated against the 575-pair corpus (`scripts/analysis/probe_575_capped.py`, 24 min wall, 0 Cloudflare blocks). Found B1 hit rate 82.4% (vs docs' 92.9%) and B10 had 0% miss recovery on its target cohort. Diagnosed 4 distinct miss classes via `scripts/analysis/experiment_miss_recovery.py` (4-variant URL A/B harness): (1) `locationId=country_4` excludes burial-unknown records — confirmed on memorial 285269207 (John Pate, Texas death, "Burial Details Unknown"); 486 US-only results push the truth out of the top 20 while 1,182 global results keep it at rank ~9. (2) `exactspelling=true` excludes spelling variants (Peter Rozell vs Rozzell). (3) Parser reads only page 1 (20 results) — truth often on page 2+ (William Ritter at rank ~47 of 486). (4) Birth-year proxy `death_year - 65` is rough; tight year filters exclude truth when off by 5-15 years. **Conclusion: B10 and the bias do not address any of the 4 miss sources.** B10's `birthyearfilter=3` is *tighter* than B1's `=1`, making the proxy problem *worse*. The bias is +0.05 to scoring, can't recover URL-filter excludes. Follow-up issues filed: **#146 (parallel FaG search via proxy pool)** to bypass L1 throttle; **#147 (pagination in `scripts/fag/parser.py`)** for Miss Class 3. Also: URL-shape validator `scripts/analysis/validate_strategy_urls.py` (11 strategies × 20 records, 9 min wall) confirms no URL-shape bugs in the 11-strategy ladder; F1b-deathyear has the best single-strategy hit rate (30%) on the pre-1851 cohort sample.

### Fixed

- **Issue #144 fix layer: parser rewrite + audit heuristic refinements.** Three interlocking changes dropped the suspicious-death-date audit from **1,151 → 445 flagged** (target <500 met). (1) `scripts/ingest/red_ink_ocr_pilot.py` parser: (a) added a year-only fallback for OCR-garbled DECEASED stamps where DATE_PATTERNS miss but a bare 4-digit year sits in the keyword's ±60-char window — recovers ~212 EMPTY_BUT_STAMP_PRESENT records; (b) added a GRANTED-stamp-aware candidate pre-filter that drops 1915/1865 candidates when a GRANT/REJECT/FILED stamp was stripped by `strip_form_lines` (regardless of candidate count) — the prior gate only fired for `len(candidates)==1`; (c) raised MAX_YEAR 1955 → 1965 in both the parser and `enrich_pensioners_with_death_dates.py` (was inconsistently 1940/1955 — silently dropping 1941-1965 deaths like Brock, William d.1956). (2) `scripts/audit/audit_suspicious.py`: (a) rewrote H2 GRANTED_PICK to use the parser's chosen window instead of any window containing the cached year (eliminates false positives where a legitimate death year 1916 also appears in a GRANTED stamp on the same card); (b) tightened H3 NUMERIC_SUBSTITUTION to require the cached year itself has no death keyword nearby (was flagging any ±1 variant with any death keyword on the card, including unrelated widow-marriage stamps). (3) `scripts/ingest/easyocr_pass.py`: added `--gpu` / `--no-gpu` flags with `torch.cuda.is_available()` auto-detect (was hard-coded `gpu=False`; the 21-93h serial CPU ETA per `data/easyocr_runs/full_2026-07-29_resume.json` dropped to ~38 min on RTX 3050 with workers=1 — workers=2/4 OOM the 6 GB card). Result: GRANTED_PICK 1,350 → 0, EMPTY_BUT_STAMP_PRESENT 591 → 333, NUMERIC_SUBSTITUTION 264 → 84. Suite: **1,688 passed, 6 deselected, 0 failed, 0 skipped** (was 1,688 before fixes — all parser tests still pass). Sidecar refreshed but NOT promoted (`docs/research/digitalprairie/ok_pensioners.with_death_dates.json` updated; `ok_pensioners.json` source untouched per CHANGELOG convention). Spot-check precision estimated ~85-90% on the 78-card sample (28 agree, 4 disagree with 3 wins for fresh EasyOCR, 16 cached_Y_fresh_N requiring operator eyeball).

### Added

- **OCR spot-check tooling** (`scripts/ingest/spot_check_ocr.py`, `requirements-dev.txt`). Random-stratified sample of enriched pensioners (10 each from red / full-fallback / easyocr source-pass buckets) + 20 no-date-with-OCR pensioners. Copies front-scan jpgs to `data/spot_check/<ts>/cards/`, re-runs EasyOCR on each (GPU, ~0.6 cards/sec on RTX 3050), emits `manifest.json` + `summary.md` side-by-side with cached `find_death_date` output + fresh parser verdict. Designed for human eyeball review against the actual card images to catch wrong-year assignments that pass every audit filter. First 8-card eyeball on 2026-08-01 already surfaced two genuine parser misses (Milburn 1936-11-24, Williford 1940-01-01) — both empty in sidecar, both confirmed by image. `requirements-dev.txt` pins torch+easyocr; the runtime tree stays clean per `requirements-ci.txt` header.
- **Suspicious-death-date audit script** (`scripts/audit/audit_suspicious.py`). Read-only re-audit of all 7,709 pensioners using 4 heuristics over the cached OCR text: `GRANTED_PICK` (cached year sits in a GRANTED/REJECTED/FILED stamp window without death keyword — parser picked wrong stamp), `EMPTY_BUT_STAMP_PRESENT` (death_year empty + DECEASED/DEATH/DIED substring found), `NUMERIC_SUBSTITUTION` (±1-digit variant of cached year has death keyword nearby), `FILE_DATE_TOO_FAR` (filed_year − death_year > 6, widows excluded). Emits JSON + markdown report under `data/audit_runs/`. 2026-08-01 run flags 1,140 unique pensioners (15% of corpus); ~83% precision on 6-card eyeball sample (5/6 real bugs). Tracked as #144 — parser-fix work is follow-up.
- **Thumbdrive copy script now ships `.git/`** (`scripts/copy_to_thumbdrive.sh`). Added `cp -a "$ROOT/.git" "$DEST/.git"` before `checkout-index` so the target is a true working clone (history, refs, hooks, packed objects, ~13 MB). Footer test-count updated `1674` → `~1,381` to match `AGENTS.md`.

- **EasyOCR config-driven launcher + resume script** (`scripts/ingest/easyocr_launch.py`, `scripts/ingest/easyocr_resume.py`, `tests/test_easyocr_launch.py`, 17 new tests). The full EasyOCR death-date enrichment run was started 2026-07-29 21:01 UTC and interrupted 2026-07-30 08:01 UTC (no traceback; hard kill at i=632/6003). Resuming it required (a) a way to capture the run's exact CLI args as a reproducibility artifact, and (b) a single command an agent can run to re-launch the run with the same config. Solution: externalize the run config to a JSON file at `data/easyocr_runs/<runname>.json` and shell out to `easyocr_pass.py` with the reconstructed argv. Two new scripts:
  - `easyocr_launch.py` — reads JSON config, fills missing keys from `easyocr_pass.py`'s argparse defaults, writes the resolved config back to disk (atomic rename), shells out to `easyocr_pass.py`. The config IS the artifact. `--dry-run` prints the resolved argv + saves the filled config without launching. The slice-to-canonical safety guard from `easyocr_pass.py:50e7ee4` is mirrored here: if the config's output resolves to the canonical `data/cards/red_ocr_results.json` but the in-memory input record count looks like a slice (< 1000), the launcher refuses to run unless `in_place=true` was set explicitly (exit 2).
  - `easyocr_resume.py` — single command to resume any saved run. Auto-promotes `in_place=true` when the config's output resolves to the canonical file (safe because `easyocr_pass.py`'s skip rule on `easy_text` is the resume handshake). Sidecar configs are not promoted. Recipe: `python scripts/ingest/easyocr_resume.py data/easyocr_runs/<runname>.json`.
  - **`easyocr_pass.py` is unchanged** (per the external-launcher decision): the in-flight run's CLI surface stays the single source of truth; the launcher reads it via `build_argv()` and stays in lockstep via the test suite.
  - Saved artifacts committed: `data/easyocr_runs/full_2026-07-29_resume.json` (the interrupted run's exact config + provenance block: started/interrupted_at/last_heartbeat/last_pid/files_at_interruption/resume_skip/eta_at_resume) and `data/easyocr_runs/slice5_smoke.json` (5-record smoke-test config used to verify the launcher end-to-end). Both reproducibly launch the same `easyocr_pass.py` argv the in-flight run used.
  - Smoke-tested end-to-end: 5-record EasyOCR slice (pcid 10035, 10723, 11110, 11113, 11116) completed in 74.6s with rc=0 (5/5 ok, 1 new date, 0 fail) via `easyocr_launch.py data/easyocr_runs/slice5_smoke.json`. Resume dry-run on the full 6003-record config reports `already_done=836/9436` from the interrupted run.
  - 17 new tests in `tests/test_easyocr_launch.py` pin: config round-trip, default filling for every key, flag inclusion/exclusion, sidecar-vs-canonical resume mode, clobber guard firing, full-run-launcher exit code, skip-resume count, schema freeze in docstring. Full suite: 1598 passing, 4 skipped (was 1581+17 = 1598).

- **EasyOCR run checkpoint watcher** (`scripts/ingest/easyocr_checkpoint.py`). Polls `red_ocr_results.json` every 60s, fires an atomic git commit + CHANGELOG bullet when a configured threshold (25 / 50 / 75 / 100%) is crossed. Idempotent via `data/easyocr_runs/checkpoints.fired.json`. At 100% the subject reads "L3 complete" instead of "at 100%". Auto-pushes on commit (opted-in via user). At run completion the watcher exits cleanly. Plan: defer hand-curation of the L3 missed-set (~3316 records with `easy_text` but no extracted `death_date`) to a separate recovery slice — filed as issue #143.

- **L3 EasyOCR run complete + sidecar refresh**. Resumed from i=632/6003 (commit `bef4f74`) and ran to completion: **5369/5369 records, 0 fail, 71079s wall (~19h45m)**, 1830 new death dates extracted by EasyOCR pass. Final pass breakdown on `red_ocr_results.json`:
  - `easyocr`: 2008 records
  - `full-fallback` (Tesseract): 1749 records
  - `red` (Tesseract red-ink mask): 1482 records
  - Total with `death_date`: **5239/9436** (55.5%)
  - Missed-set (`easy_text` present, no `death_date`): **4163** records — filed as issue #143 for a permissive-regex recovery slice. The original L0/L1/L2 baseline was 3410 dates; L3 added **+1829** (53.6% relative increase).
  - `enrich_pensioners_with_death_dates.py` re-run: **4857/7709 pensioners (62.9%)** have a `death_date_iso` (vs 3167/7709 = 41.1% before L3).
  - **L3 missed-set recovery pass** (`scripts/ingest/recover_missed_dates.py`, `tests/test_recover_missed_dates.py` 19 new tests, issue #143). Pattern-based candidate generator with confidence scoring + the L3 anti-keyword guards. Run on the 9436-record canonical: 207 records produce 242 candidates, all in the review tier (none at 0.7+ confidence). **Honest result: 0 expected lift on the real data** — the missed-set's "DECEASED" stamps have no parseable date in the surrounding text; the OCR garbled the date beyond regex recovery. Posted the finding to #143 with the recommendation to close. The recovery pass ships as a clean utility in case an L4 preprocessor (image upscale + ROI crop the top-right) exposes a cleaner text later.
  - Suite: **1617 passing, 4 skipped** (was 1598+19=1617 ✓).
  - **Audit script externalization** (`scripts/audit/audit_launch.py`, `tests/test_audit_launch.py`, `data/audit_runs/2026-07-31_l3_postrun.json`, 7 new tests, issue #141). Applies the same JSON-config + atomic-rename pattern as `easyocr_launch.py`. The `audit_death_dates.py` argparse surface gains `--enriched / --enrichment / --ocr / --out-json / --out-md` flags (defaults preserved for back-compat). Saved artifact captures the run's effective config + a provenance block (intent + expected finding categories). Smoke: dry-run + full run with the L3 post-run config; reports write to the canonical paths. Suite: 1624 passing, 4 skipped.
  - **Download script externalization** (`scripts/ingest/download_launch.py`, `tests/test_download_launch.py`, `data/ingest_runs/applications_2026-07-29.json`, 7 new tests, issue #142). Same JSON-config + atomic-rename pattern as the EasyOCR + audit launchers. `download_application_images.py` already had full argparse, so no refactor was needed. Saved artifact captures the full-corpus config + provenance (smoke-run timing, throttle rationale). Dry-run verified the argv wires the same flags the in-flight application download used. Suite: 1631 passing, 4 skipped.
  - **E2E ground-truth tests: zero skipped.** Replaced the operator-private `C:/tmp/ground_truth.csv` (issue #91, 4 skipped tests) with a bundled public fixture at `tests/fixtures/ground_truth.csv` built from `dixiedata.db` joined to `ok_pensioners.json` (566 FAG records, 240 join to OK pensioners). New `scripts/audit/build_ground_truth_fixture.py` regenerates the fixture; the 4 e2e tests now run by default against the bundled GT (5 fast fixture-validation tests in the default suite + 4 live searcher e2e tests gated by `@pytest.mark.diag`). The live tests run the full Playwright+stealth searcher (~12 min for 10 records, 38 min for 50), so they stay diag-only; default suite stays at 25s. Two bug fixes landed along the way: `scripts/pipeline/run_unified.py:1525` had an `UnboundLocalError` on `batch_cfg` when `--config` was absent (initialized to `None`); the GT CSV schema had to match the searcher's expected field names (`id` + `application_number`, not `pensioner_id` + `pension_number`) so the searcher resolves pensioner records correctly. Suite: **1642 passed, 6 deselected, 0 failed, 0 skipped.**
  - **Issue #139 follow-up: re-enrich driver now processes `easy_text` + L1 stamp-fragment rejection.** Two interlocking fixes to the death-date extraction audit (the 2116-record `NO_KEYWORD_BUT_DATE` bucket that had been growing through L0→L3). The re-enrich driver `scripts/ingest/re_enrich_from_ocr.py` now scores death-date candidates across all three text sources (`red_text` / `full_text` / `easy_text`) and picks the best by `(full-date > year-only) x (near_death_keyword) x (mentions_soldier_name)`. Previously only red+full were re-parsed, so the 2008 L3 EasyOCR results held stale L0-era parses. `find_death_date` also gained a stamp-fragment guard: when the cleaned text has 1915 (or 1865) as the ONLY year candidate AND the line-stripper dropped chunks containing GRANTED/REJECTED (or war-end keywords), the candidate is rejected. This catches the post-strip orphan pattern (`ON7=1915`, `0CT7 - 1915`, etc.) where the GRANTED keyword drops but the orphan date survives. Audit impact: `NO_KEYWORD_BUT_DATE` 2116→1488 (-628, -30%); `1915` in that bucket 136→57 (-58%); `1865` in that bucket 42→6 (-86%); `FULL_DATE_BUT_YEAR_ONLY` 80→38 (-52%). The 850+ records that moved from `NO_KEYWORD_BUT_DATE` to `WIDOW_BUT_NO_DATE` (1052→1901) are the EasyOCR-correct reclassification: their year-only picks were wrong, so the records are now honestly flagged for manual review instead of carrying a wrong death year downstream. Re-enrich side-effects are tracked in `data/cards/red_ocr_summary.json`; `data/audit_death_dates_report.json` regenerated; `docs/research/digitalprairie/ok_pensioners.with_death_dates.json` sidecar refreshed. Suite: **1658 passed, 6 deselected, 0 failed, 0 skipped.**
  - **Issue #139 final fix: fuzzy death-keyword matcher + audit reads `near_death_keyword` from `death_date` subfield.** The remaining 1488 `NO_KEYWORD_BUT_DATE` records dropped to **162** (-89.6%, total 2116→162; -1954 records). Two interlocking fixes: (1) the audit was reading `near_death_keyword` at the TOP level of OCR records, but the re-enrich driver writes it INSIDE `death_date` — the audit now reads from both locations for backward compat; (2) OCR commonly mis-reads the word DECEASED on these pension cards (spot-checked 30 residuals: `peceased`, `Dededsed`, `Decea sed`, `Deceasea`, `DBCBASBD`, `DSEEASED`, `DECZASED`, `DECHASHD`, `vecoa`, `Amceased`, etc.). New `fuzzy_death_keyword()` accepts canonical forms + a hand-curated OCR misread set + a Levenshtein-distance-2 fallback for novel variants. Wired into `find_death_date` keyword_spans, the per-window filter bypass, `process_image`'s annotation, and the re-enrich driver's `_annotate`. Enriched records with death keyword: 0 → 2747 (new signal). Total findings 3272 → 2053 (-37%). Remaining 162 are genuinely hard (real GRANTED 1915 stamp residuals + a few `HONEB`-class misreads); correct candidates for manual review via the v2 viewer (wired in #140 below). Suite: **1665 passed, 6 deselected, 0 failed, 0 skipped.**
  - **Issue #140: wire pension application images into the viewer (after download job finishes).** The application-image download job (`scripts/ingest/download_application_images.py`, commit `1be7059`) had already completed: 7451 ok + 100 pre-existing + 158 http-404 = 7551 application jpgs in `data/cards/applications/`, ~1.5 GB. The viewer + bundle now consume them. `build_pensioncard_viewer.py` loads application jpg names keyed by **pensioner_id** (not pensioncard_id — the digitalprairie apps live at `pensions:{id}/...`, cards at `pensioncard:{pcid}/...`); records carry new `application_image` + `application_available` fields; each letter page has a parallel `letters/{L}/applications/` subdir with the matching pensioner jpgs; the lightbox toolbar has a Card ↔ Application toggle (`c`/`a` keyboard shortcuts); each record row shows an `App` / `no app` badge. `distribute_pension_viewer.py` ships `data/cards/applications/` in the full bundle + `letters/{L}/applications/*.jpg` in per-letter zips + `download_summary_applications.json` as shared metadata; the slim index zip excludes the heavy jpg trees. Round-trip on the real corpus: letter A = 248 records, 246 application images copied (2 are the genuine 404 cases). Suite: **1674 passed, 6 deselected, 0 failed, 0 skipped.**
  - **Issue #138: death-year-aware candidate scoring.** The FaG searcher previously had only 3 death-year proximity tiers (0y=0.5, 1-2y=0.4, 3-5y=0.2) and **no widow-specific logic** when the soldier's death year was known. Two fixes in `scripts/fag/scoring.py`: (1) added two more tiers (6-10y=0.1, 11-20y=0.05) for smooth decay; (2) added a widow-specific override that fires when `is_widow and local_dy` are both set — a candidate who died **before** the soldier's death year (impossible for the widow) gets `death_score=0`, while a candidate who died 0-60 years **after** the soldier gets a directional proximity bonus (0.45 → 0.15). The J13 ACW soft gate still applies unchanged. Tests added to `tests/test_date_filter_j13.py`: 7 new cases covering the proximity tiers, the widow directionality, and the J13 regression (out-of-window candidates still carry `_date_penalty`). Two `test_issue_138_*` cases simulate realistic 4-way contests to verify the right candidate wins. Suite: **1681 passed, 6 deselected, 0 failed, 0 skipped.** Out of scope for this slice: the CalibrateClassifier retrain (training is on labeled benchmark, separate work), the bulk re-search of all 7709 pensioners (operational step), and the strategy-ladder changes that #137 will own.
  - `audit_death_dates.py` re-run: 3272 findings (was 3437 pre-L3). `NO_KEYWORD_BUT_DATE` 1648 → 2116 (+468, the parser got more permissive on EasyOCR text — most are real but missing death-keyword). `WIDOW_BUT_NO_DATE` 1558 → 1052 (-506, the lift). `FULL_DATE_BUT_YEAR_ONLY` 35 → 80 (+45, mechanical downgrade that #143's recovery pass should also target). The 2116 NO_KEYWORD_BUT_DATE entries are the audit's view of the #143 missed-set; the recovery pass's permissive regex (with anti-keyword guards preserved) is the right next step.
  - Committed: refreshed `data/cards/enrichment_report.json` and `docs/research/digitalprairie/ok_pensioners.with_death_dates.json` sidecar. The sidecar is the developer-review step (CHANGELOG convention: SOURCE UNTOUCHED; cp the sidecar over `ok_pensioners.json` after manual review to activate). Suite unchanged: 1598 passing, 4 skipped.

- **Dirty-tree cleanup: ship the audit + download scripts, vendor Leaflet 1.9.4 with LICENSE, drop probe subset, file follow-ups**.
  - `scripts/audit/audit_death_dates.py` — committed with one real bug fixed: line 38 had `sys.insert = sys.path.insert` (attribute assignment, not function call) which shadowed the real `sys.path.insert` for any later importer in the same process. Removed. The script's 10 finding categories (OUT_OF_RANGE_YEAR, ISO_YEAR_MISMATCH, FULL_DATE_BUT_YEAR_ONLY, NO_KEYWORD_BUT_DATE, WIDOW_BUT_NO_DATE, CONFLICTING_DATES, SUSPECT_MONTH_DAY, PASS_RED_BUT_LOW_TEXT, DUPLICATE_PENSIONER_ID, NON_NUMERIC_YEAR) read three JSON sources and emit JSON + MD reports. `--strict` exits 1 on any finding for CI gating. Filed issue #141 to externalize to a launcher + JSON config (same pattern as `easyocr_launch.py`) when a second use-case shows up.
  - `scripts/ingest/download_application_images.py` — Each pensioner has two distinct Digital Prairie items: the **application** (the original pension request form, `pensions:{id}` in the IIIF URL) and the **pension card** (3×5 index card, `pensioncard:{id}`). Previously we only had the cards; this script pulls the applications. Sequential + 0.25s throttle (digitalprairie is a static CDN with no observed rate limits — the 2.5s FaG throttle does not apply here), resume-safe via per-record `<id>.jpg` existence check, 404s logged as `missing_source` (some pensioners genuinely have no application record) rather than blocking the run, `--refresh` to force re-download, `--limit N` for dry-runs, `--throttle N` to override. Output to `data/cards/applications/<id>.jpg` (~190 KB each, ~7709 files → ~1.5 GB). Per-record summary flushed every 100 records; 5-minute heartbeat. Background smoke-tested on the first 100 records (104.3s, 0 misses, 0 fails). Full corpus run is in `data/applications_download.log` (running). Filed issue #142 to externalize this script to a launcher + JSON config when a second use-case shows up.
  - `scripts/ingest/vendor/leaflet.min.js` + `leaflet.css` + `leaflet-images/` (5 PNGs) + `LICENSE-Leaflet-1.9.4` — Leaflet 1.9.4 vendored offline (BSD-2-Clause, Volodymyr Agafonkin + CloudMade, 2010-2023). Used by `scripts/view/v2.html` to replace the OpenSeadragon viewer (WebGL init failure on the test environment, see the Leaflet OSD-swap bullet under `### Fixed` below). LICENSE pinned to the v1.9.4 release tag.
  - `docs/research/digitalprairie/ok_pensioners_g10.json` (10 records, 40KB) — **dropped**. Probe subset of the canonical 7709-record `ok_pensioners.json`; not data worth keeping. The canonical file is already committed.
  - `.gitignore` — added ignore rules for the audit + application-downloader output files (`data/audit_death_dates_report.{json,md}`, `data/cards/download_summary_applications.json`, `data/easyocr_full_run.pid`). Existing carve-outs (stealth-verification evidence, runtime logs, applications dir) preserved.

- **Extraction-audit script** (`scripts/audit/audit_death_dates.py`) — cross-references `ok_pensioners.with_death_dates.json` against `red_ocr_results.json` to surface suspicious death-date extractions. Five finding tags: `OUT_OF_RANGE_YEAR`, `ISO_YEAR_MISMATCH`, `FULL_DATE_BUT_YEAR_ONLY` (75 records: OCR text has full `Month Day, Year` but `death_date_iso` was downgraded to bare year — pure mechanical fix possible), `NO_KEYWORD_BUT_DATE` (1349 records: date extracted but no death-context keyword — includes both legit dates and confirmed OCR-misreads like `id=6 Alberty, Andrew J.` where `red_text` reads `"Deceased, 4-12-1032"` but the record has `1933-04-13`), `WIDOW_BUT_NO_DATE` (1975 widows without extracted dates — mostly legit), `DUPLICATE_PENSIONCARD_ID` (22 cases: 21 are 2-pensioner pairs, 1 is 43 pensioners sharing `pensioncard_id=10528` — a digitalprairie API placeholder, not our bug), `SUSPECT_MONTH_DAY` (2). Reports saved at `data/audit_death_dates_report.{json,md}`; `--strict` exits 1 if any findings (for CI gating). Filed as issue #139 for triage.

- **`scripts/ingest/re_enrich_from_ocr.py`** — re-extracts death dates from the **cached** OCR text (no re-OCR) using the updated `find_death_date` parser. The audit identified five parser-side root causes that the original `red_ink_ocr_pilot.py` had: tight ±30-char filter window (missed "came to Oklahoma Territory 1912"), MAX_YEAR=1940 (rejected legit 1941-1955 widow deaths), date regex requiring whitespace before year (missed "June 5,1902"), no 2-digit-year parser (missed "Deceased 1-26-25"), no MARRIED_RE / FILED_RE filters (marriage dates and filing-stamp dates were being picked as deaths). All five fixed in `red_ink_ocr_pilot.py` plus a re-driver over the cached OCR. After running this + the downstream `enrich_pensioners_with_death_dates.py`: total enriched pensioners **3167 → 3676 (+509, +16%)**, with **full-date coverage up from ~30% to ~46% of all pensioners**; audit findings: `WIDOW_BUT_NO_DATE` 1975 → 1765 (-210), `FULL_DATE_BUT_YEAR_ONLY` 75 → 35 (-40), but `NO_KEYWORD_BUT_DATE` 1349 → 1648 (+299) — the parser is now more permissive and picks up dates that lack a death-keyword nearby; the next step is the heuristic line-stripping + EasyOCR pass to filter out the false positives. 16 new tests in `tests/test_red_ink_ocr_pilot.py` pin the new parser behaviors.

- **L1 — line-strip form fields** (commit `c7c66d8`): `red_ink_ocr_pilot.find_death_date` now calls `strip_form_lines()` before scanning, which drops lines matching `REJECTED | GRANTED | Filed <date> | By letter | Q/C of | cancelled | approved | filling` and protects lines containing `deceased|died|death|d.o.d.|date of death`. Splits on `\n`, `|;`, 2+ spaces (Tesseract's form-row separator signature), and sentence ends. After L1: total enriched pensioners **3676 → 4053 (+886 from original, +12% over L0)**, `WIDOW_BUT_NO_DATE` 1765 → 1558 (-207), `FULL_DATE_BUT_YEAR_ONLY` 35 → 37, `NO_KEYWORD_BUT_DATE` 1648 → 1818 (+170 — most are soldier cards where the OCR didn't capture the 'Deceased' stamp; these need EasyOCR not regex fixes). 6 new tests pin the new behavior. Suite: 1564 passing, 4 skipped.

- **L2 — precision-focused refinements**: the score function in `find_death_date` now uses a 5-tuple sort (added median-cluster penalty) so when no death keyword is present the year closest to the median of all candidate years is preferred; precision filters add (1) reject the year 1915 when other year candidates exist (almost always the GRANTED stamp), (2) reject 1865 when other years exist (war-end parole/surrender), (3) require the soldier's name within ±120 chars of a year candidate on widow cards when no death keyword is present. **Trade-off**: total enriched pensioners 4053 → 3748 (-305), but `NO_KEYWORD_BUT_DATE` 1818 → 1648 (-170) and total findings 3437 → 3401. Net precision win — a wrong year picked causes false FaG matches downstream, while a missed year leaves the pensioner in the manual-review queue. 6 new tests pin the new behavior. Suite: 1570 passing, 4 skipped.

- **L3 — EasyOCR second pass** (commit `b001132`): installed `easyocr` (with `torch`, `scikit-image`, `opencv-python-headless` deps, ~1.5 GB) and added `scripts/ingest/easyocr_pass.py` driver that re-OCRs records where Tesseract's red-text pass returned empty (highest-value subset). Ran a 100-record priority slice (widow records with empty `red_text`) to benchmark: 24 min CPU, 19 'new dates' extracted, **1 actually correct widow-death-year** (1% precision). The death stamp is hand-written or small-font in the top-right of the card; both Tesseract and EasyOCR read it inconsistently. EasyOCR reliably recovers QC/Filed/war-end dates that the red-mask missed, but those are filing dates, not deaths. The full 7h run was not justified; driver + deps are shipped so a future L4 preprocessing pass (image upscale + ROI crop the top-right) can re-OCR with better image prep. Suite unchanged: 1570 passing, 4 skipped.

- **L3 follow-up: 50-record mixed slice + safety guard** (commit `50e7ee4`). Ran a 50-record seed=42 random slice (widows + soldiers) with the existing settings: 16 min, 30 raw new dates, **~5 real death dates (~10% real lift)**. Higher real lift than the 100-widow slice because soldier cards have cleaner DECEASED stamps that EasyOCR reads well. The slice surfaced the same parser bug seen on the 100-slice: when no death keyword is present, `find_death_date` falls back to the first plausible date and grabs filing/correspondence dates ("Filed 6/14/15", "Letter 3/6/23 gives Address", "Changed from Hollis to Gould 9/18/19"). **Do not start the 7h full EasyOCR run until this is patched and a clean slice confirms the fix.** Also patched `easyocr_pass.py` with a safety guard: `--output` no longer defaults to the canonical file path; new `--in-place` flag is required to overwrite `data/cards/red_ocr_results.json`; guard refuses to proceed if `--input` resolves to the canonical file but contains <1000 records and `--in-place` wasn't given. This prevents the 2026-07-29 incident where a 50-record slice silently clobbered the 9436-record canonical file (recovered intact from a `%TEMP%\extract_conflict\` test-run backup). Slice protocol + locked settings frozen in `docs/learnings/2026-07-29-easyocr-slice-protocol.md`. Suite unchanged: 1570 passing, 4 skipped.

- **L3 follow-up: address-change anti-keyword filters + widow-strictness fallback removal** (commits `5f4a296`, `07b2714`, `3bfa0c0`). Two parser fixes for the false positives surfaced by the 50-slice: (1) new `ADDR_CHANGE_RE` and `LETTER_DATE_RE` regex catch "gives Temp Address", "Changed from X to Y", "o/c X/X/XX", "Letter M/D/YY gives Address" patterns, both in `strip_form_lines` (drops whole chunks) and as inline window filters in `find_death_date`; (2) removed the L2 widow-strictness `'only candidate'` fallback that let single-candidate dates through when the soldier name wasn't nearby. Re-slice (v3, fresh canonical data): 12 new dates (down from 30), 6 real deaths confirmed by hand (pcid 803, 3263, 3498, 4742, 6191, 10969 — all with explicit `Deceased M-D-YYYY` stamps), 3 false positives (pcid 2285/4652/5400 where Deceased stamp OCR was too garbled and parser fell back to GRANTED/FILED stamp dates), 3 correctly returned None (no Deceased stamp present at all). **Real lift: 12% of records**, **false-positive reduction: 5x** (3 vs 15 in v2). 11 new tests pin the new behavior. Suite: 1581 passing, 4 skipped. **The 7h full EasyOCR run is now safe to start.** Estimated ~500-700 real death dates recovered from the 4891 no-date records.

### Fixed

- **Pension-card viewer: swap OpenSeadragon → Leaflet.** OSD's WebGL tile renderer was failing in this environment (`Error creating texture in WebGL. undefined` in console; canvas stayed transparent until the user clicked the lightbox center). Replaced with Leaflet 1.9.4 + `CRS.Simple` + `L.imageOverlay` (`lib/leaflet.min.js` 144 KB, `lib/leaflet.css` 14 KB, 5 marker PNGs in `lib/leaflet-images/`). Pan/zoom UX preserved (wheel zoom, drag-pan, double-click zoom, +/−/0/←/→ keys, s to download the original JPEG, esc to close, f for fullscreen, r rotates the `<img>` via CSS transform). Image now renders immediately on click. OSD-related files cleaned out of `lib/` (the `openseadragon-images/` sprite dir + `openseadragon.min.js` are no longer copied; old leftover files removed from disk).

- **Pension-card viewer: relative-path fix for Layout A.** Layout A (commit 8ee229d) put letter pages at `letters/{L}/viewer/{L}.html`, but `LETTER_PAGE_TEMPLATE_HEAD` / `LETTER_HEADER_HTML` / `render_letter` still emitted `../lib/`, `../B/viewer/B.html`, `../../../index.html` (one `../` too few). Updated to `../../lib/`, `../../../B/viewer/B.html`, `../../../index.html` so all relative URLs resolve from the `letters/{L}/viewer/` directory. The `app.js` script tag dropped `defer` so it runs synchronously before Alpine evaluates `x-data="letterApp()"` — Alpine with `defer` was booting before `letterApp` was defined, producing `letterApp is not defined` errors on every expression. Tests in `test_build_pensioncard_viewer.py` updated to pin the new `../../../lib/...` paths.

- **Distributor rename bug.** `scripts/ingest/distribute_pension_viewer.py:write_letter_zip` failed with `FileExistsError` whenever the partial's destination already existed (a common case: a previous crashed run left the final `.zip` and the new run's `.partial` couldn't be renamed onto it). Switched `partial.rename(out_path)` → `partial.replace(out_path)` with a `WARNING` log so re-running after a crash no longer wedges. Dry-run output fixed too — synthetic `START_HERE.html` entries (no source file) used to crash `p.stat().st_size`; now sized via `len(START_HERE_HTML.encode("utf-8"))`.

- **Pension-card viewer: `x-show` vs CSS `display: flex` cascade.** `.lightbox-overlay` and `.records` both declared `display: flex` in CSS, which won over Alpine's inline `display: none` from `x-show`. Result: the lightbox was visible from initial render and the records container showed even when empty. Switched both `x-show="..."` bindings to `x-show.important="..."` so Alpine applies `display: none !important`. `.empty-state` already worked (no `display` declared) — left alone.

### Added (prior session, 2026-07-25)

- **`scripts/ingest/distribute_pension_viewer.py --by-letter`** — slice the pension-card bundle by surname letter. Each letter emits its own zip (`pension-viewer-bundle.A.zip`, `.B.zip`, …, 2 MB for Z up to ~500 MB for B) containing only the jpgs for that letter + the matching `{letter}.html` / `{letter}.json` viewer page + the shared metadata so the reviewer can browse a single letter offline without pulling the 4.8 GB monolith. A slim `pension-viewer-bundle.index.zip` is also emitted with the full viewer (every `{letter}.html`, `index.html`, `all.json`) and ZERO images so reviewers can browse the surname index before deciding which letter-zips they need. Pensioners with no surname initial (e.g. `Mrs. So-and-So`, `A. J.`) are routed to `pension-viewer-bundle._.zip` using the viewer's existing `_.html` page, so no jpgs are silently dropped. `--letters A,B,C` restricts to a subset; `--dry-run` lists projected sizes without writing. The previous monolithic 5-part bundle (`--split 1024`) is unchanged. 12 new tests in `tests/test_distribute_pension_viewer.py` pin the contract (full suite still 1535 passing, 4 skipped).

- **Layout A + Alpine.js + OpenSeadragon viewer.** The pension-card viewer is rebuilt end-to-end:
  - **Layout** — `data/cards/viewer/{index.html, all.json, lib/, letters/{L}/{viewer/{L}.html, A.json, img/}}`. Index is now at the viewer root (1 hop up from the previous `data/cards/viewer/index.html`). Each letter is a self-contained `letters/{L}/` subfolder; extracting all 26 by-letter zips into a single folder produces zero conflicts (verified — 9560 unique paths written, 0 overwrites, 0 byte-level conflicts). The bundler's `--by-letter` paths and tests track the new layout.
...
- **Full red-ink OCR run + scoring-integration work filed in #138.** Production-scale run on all 7709 pensioners extracted death dates for 3167 (41.1%) with ~70% estimated precision. New scripts: `red_ink_ocr_watchdog.py` (auto-restart on silent OCR deaths), `enrich_pensioners_with_death_dates.py` (writes sidecar `ok_pensioners.with_death_dates.json` — source untouched), `build_pensioncard_viewer.py` (per-surname-letter HTML+JSON viewer with lazy-loaded images, ~17 MB), `distribute_pension_viewer.py` (atomic-write zip bundler for Google Drive/HDD backup, produces 5 × 1 GB parts). Key bug fix: widow-aware date picker (issue #145) — on widow cards the red stamp is the WIDOW's death date; the typewritten body text contains the SOLDIER's death. Pipeline now prefers candidates whose context window mentions the soldier's last name. Verified on `Baker, Dora` (widow of `John Stephens Baker`) — previously picked widow's death 1928-07-18, now correctly picks soldier's 1915-02-26. **The extracted death_year field is in the JSON but the FaG searcher scoring does not yet use it for ranking** — see issue #138 for the integration work (date-window narrowing, proximity bonus, classifier retrain, bulk re-search). See `docs/learnings/2026-07-28-red-ink-ocr-full-run.md` for the full build writeup, including the bugs that bit us (uint8 overflow in red mask, ascending-tuple sort inversion, IIIF compound-object 501s, silent OCR process deaths).

- **Full red-ink OCR run on all 7709 pension cards.** 9436 page images downloaded (~50% two-sided); 3231 page-sides yielded a candidate date; **3167 pensioners (41%) enriched with death_year + death_date_iso** in sidecar file `docs/research/digitalprairie/ok_pensioners.with_death_dates.json`. SOURCE UNTOUCHED — developer must `cp` the sidecar over `ok_pensioners.json` after review to activate. The `death_year` field flows automatically through `scripts/blackboard/projector.py:66` into `decision_policy`'s auto-accept threshold. No FaG searcher changes needed.

- **Widow-aware death-date disambiguation (issue #145).** The previous pipeline didn't distinguish widow pension cards from soldier pension cards. On widow cards the red-ink stamp often records the WIDOW's own death date, not the soldier's; the typewritten body text reliably contains the soldier's death. New logic: pipeline reads `spouse_name_raw` to detect widow cards, then per-OCR-pass and per-card scoring prefers candidates whose context window mentions the soldier's last name. Verified on `Baker, Dora` (widow of `John Stephens Baker`) — previously picked widow's death 1928-07-18, now correctly picks soldier's 1915-02-26 (`He died February 26 1915 in Pushmataha County`). Per-card scoring priority: `mentions_soldier_name > near_death_keyword > kind=date > earlier_year`.

- **`scripts/ingest/red_ink_ocr_watchdog.py`** — auto-restarts the OCR script when it dies (silent OCR deaths happened multiple times during this run; no exception, no error trace). Polls every 30s, restarts with resume support, exits after 10 idle polls.
...
- **Verified-spouse link in v2 review UI** (#137): top candidates with `_spouse_verified` now render a clickable 👫 link to the captured spouse's FaG memorial page (e.g. Haggard, Mary → Denny L. Haggard at `findagrave.com/memorial/40964275/denny_l-haggard`). `normalizeRecordV2` lifts `spouse_match` from the top candidate's `evidence.raw` onto a `spouse_match` field on the common candidate; a new `spouseLink(candidate)` helper builds the URL when `captured_memorial_id` is present. Tooltip surfaces `match_strength` so reviewers can judge trust. Across the ha run, 87 of 300 candidates render the spouse link. Four new Playwright tests cover link presence/absence + URL + tooltip.
- **`scripts/distribute.py`** — bundle run folders into portable .zip bundles for sharing/review. CLI takes `--group NAME=run1,run2` (repeatable) or `--groups-file path/to/config.txt`; each group becomes one `<NAME>.zip` containing `<runname>/view.html` + `<runname>/results.jsonl`. Source run folders are never modified. Layout is extraction-safe: unzip anywhere and double-click view.html to start reviewing (v2 fetches its sibling results.jsonl on open). 14 tests pin the contract.
...
native `run_audit.jsonl` to [OCSF 1.8.0](https://schema.ocsf.io/)
format. The runner emits `run_audit.ocsf.jsonl` alongside the
native file on every run; the native file stays the source
of truth (operators can grep it directly). The OCSF sidecar
exists for SIEM ingestion (Splunk, Datadog, Elastic, etc.).

Mapping (11 native event types → 3 OCSF classes):
- 1006 Scheduled Job Activity — work_claimed, work_completed,
  strategy_ran/skipped/error, pensioner_start/end,
  observation_appended, run_summary.
- 7003 Process Remediation Activity — cooldown_set.
- 2004 Detection Finding — bot_wall_observed.

Verified on the G10 audit log: 6,592 OCSF envelopes emitted
(125 Create, 6,342 Update, 125 Complete; 2.1 MB sidecar). The
runner integration is non-fatal (logged warning on translation
error, doesn't abort the run).

Fourteen new tests pin:
- OCSF_MAPPING covers every native event type.
- class_uid + class_name are valid OCSF classes.
- Each per-class event type produces the right activity_id
  (1=Create, 2=Update, 3=Complete) and a well-formed OCSF
  envelope (class_uid, class_name, time ms, metadata,
  per-class payload).
- Unknown event types return None (no malformed OCSF line).
- `translate_audit_file` reads native + writes sidecar;
  unknown events are silently skipped.
- The G10 audit log translates end-to-end with the right
  class structure.
...
Follow-up #100 filed: OCSF event wrapping for SIEM interop
(per the standards-bundle discussion). Not blocking; only
valuable when an actual SIEM user materializes.
...
- **EasyOCR L3 checkpoint at 25%** (1808/5369 records, heartbeat rate=0.08/s eta=56016ss). Generated by scripts/ingest/easyocr_checkpoint.py.
- **EasyOCR L3 checkpoint at 50%** (2686/5369 records, heartbeat rate=0.08/s eta=43835ss). Generated by scripts/ingest/easyocr_checkpoint.py.
- **EasyOCR L3 checkpoint at 75%** (4030/5369 records, heartbeat rate=0.08/s eta=27585ss). Generated by scripts/ingest/easyocr_checkpoint.py.
- **EasyOCR L3 checkpoint at 100%** (5372/5372 records, heartbeat rate=0.08/s eta=10590ss). Generated by scripts/ingest/easyocr_checkpoint.py.
## [Released] — 2026-07-20

...
These attributes hold references to all the calling frames of every
...
Community fix (laztheripper, 2026-06): replace with empty list
`[]` so the attributes don't pin frames. The stack traces are
intended for debug diagnostics; they're not needed for normal
operation.
...
def apply_playwright_leak_fix() -> bool:
    ...
    def _sync_fixed(self, coro):
        ...
        # Community fix: was inspect.stack(0) and
        # traceback.extract_stack(limit=10). Both pin frames.
        # Use empty list so the task is fully GC-eligible when
        # done.

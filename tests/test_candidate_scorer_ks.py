...
def test_deep_refiner_enqueues_global_plan_for_ambiguous_score(store):
    ...
    # Issue #76: needs_review tier (0.6 >= 0.40, < 0.85) emits
    # B3-first-initial-fuzzy + F3-nickname on OK scope.
    assert len(observations) == 2
    # First plan: B3-first-initial-fuzzy
    plan = observations[0].payload
    assert observations[0].kind == Kind.FaGSearchPlan
    assert observations[0].caused_by == "work-refine"
    assert plan["strategy"] == "B3-first-initial-fuzzy"
    assert plan["scope"] == "OK"
    assert plan["params"]["last_name"] == "Looney"
    # Second plan: F3-nickname
    plan2 = observations[1].payload
    assert plan2["strategy"] == "F3-nickname"
    assert plan2["scope"] == "OK"
    # Verify enqueued in store
    plan_row = store.con.execute(
        "SELECT strategy, scope FROM query_plans WHERE plan_id = ?",
        (plan["plan_id"],),
    ).fetchone()
    assert plan_row == ("B3-first-initial-fuzzy", "OK")
    ...
    persisted = store.read_observations_since(None)
    observed_ids = {o.observation_id for o in observations}
    persisted_ids = {o.observation_id for o in persisted}
    assert observed_ids.issubset(persisted_ids)
